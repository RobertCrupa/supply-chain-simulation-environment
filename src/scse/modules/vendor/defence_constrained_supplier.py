...
class DefenceConstrainedSupplier(Agent):
    ...
    def __init__(self, run_parameters):
        ...
        self._item_supplier_routing = self._runtime_context.get(
            "item_to_supplier_routing", {}
        )
        self._runtime_supplier_nodes = self._runtime_context.get("supplier_nodes", [])
        self._runtime_supplier_ids = [
            s.get("id")
            for s in self._runtime_supplier_nodes
    ...
    def reset(self, context, state):
        ...
        if "item_to_supplier_routing" in context and isinstance(
            context["item_to_supplier_routing"], dict
        ):
            self._item_supplier_routing = context["item_to_supplier_routing"]
        if "supplier_nodes" in context and isinstance(context["supplier_nodes"], list):
            self._runtime_supplier_ids = [
                s.get("id")
                for s in context["supplier_nodes"]
                if isinstance(s, dict) and s.get("id")
            ]
    ...
    def compute_actions(self, state):
        ...
        if len(self._runtime_supplier_ids) > 0:
            supplier_nodes = [
                node_id for node_id in self._runtime_supplier_ids if node_id in G.nodes
            ]
        else:
            supplier_nodes = [
                n for n, d in G.nodes(data=True) if d.get("node_type") == "vendor"
            ]
        ...
        for po in purchase_orders:
            ...
            if is_supplier_disrupted(self._scenario, supplier, current_week, self._rng):
                logger.info(
                    f"Week {current_week}: {supplier} DISRUPTED — PO for {asin} x{quantity} delayed"
                )
                unfulfilled_pos.append(po)
                continue
    ...
    def _select_supplier(self, G, supplier_nodes, asin, po):
        ...
        """
        routing = self._item_supplier_routing.get(asin)
        if isinstance(routing, str):
            if routing in supplier_nodes:
                return routing
        elif isinstance(routing, list):
            for supplier in routing:
                if supplier in supplier_nodes:
                    return supplier

        destination = po.get("destination", self._factory_node_id)
